...
def capture_one_image(topic="/pylon_camera_node/image_raw",
                      out_dir="/home/marat/Desktop/test",
                      filename="frame.png",
                      timeout=10.0):
...
def get_coords(image_path):
    ...
    print("Has ArUco:", "detectMarkers" in dir(cv2.aruco))
    ...
    # Поза маркеров в системе камеры

    # Калибровочный маркер
    rvec_calib, tvec_calib, _ = aruco.estimatePoseSingleMarkers(
        corners_calib, marker_size_calib, camera_matrix, distortion
    )
    rvec_calib = rvec_calib[0].reshape(3,1)
    tvec_calib = tvec_calib[0].reshape(3,1)
    R_calib_cam, _ = cv2.Rodrigues(rvec_calib)   # ^C R_M1

    # Второй маркер
    rvec_target, tvec_target, _ = aruco.estimatePoseSingleMarkers(
        corners_target, marker_size_target, camera_matrix, distortion
    )
    rvec_target = rvec_target[0].reshape(3,1)
    tvec_target = tvec_target[0].reshape(3,1)
    R_target_cam, _ = cv2.Rodrigues(rvec_target) # ^C R_M2
    ...
    singular = sy < 1e-6
    if not singular:
        z = np.arctan2(R_m2_robot[1,0], R_m2_robot[0,0])
    else:
        z = 0
    ...
    # print("\nTarget Marker 2 Yaw Angle around Z-axis in Robot Base (degrees):")
    # print(yaw_m2_robot_degrees)
...
class MoveGroupPythonInterface(object):
    ...
    def __init__(self):
        ...
        print("TEST")
        # capture_one_image()
        # rospy.wait_for_message('/image_send_wait')
...
def main():
    ...
    try:
        ...
        # 7mm
        input(
            "============ Press `Enter` to calulate coordinates ..."
        )
        print("----------------------------------------------------------")
        print("LOADING IMAGE FROM TOPIC")
        path_to_image = capture_one_image()
        print("LOADED IMAGE!")
        print("----------------------------------------------------------")
        print("CALCULATING!")
        x,y,z = get_coords(path_to_image)
        print("CALCULATED")
        print("----------------------------------------------------------")
        print("X = ", str(x))
        print("Y = ", str(y))
